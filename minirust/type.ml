...
type mut = Ast_types.mut = Mut | NotMut
type lifetime = Ast_types.lifetime

type 'lft typ = 'lft Ast_types.typ =
...
type erased_typ = Ast_types.erased_typ
type full_typ = Ast_types.full_typ
...
let subst_fields_types sid lfts =
  let sd = get_struct_def sid in
  let subst = mk_subst sd.slfts lfts in
  List.map (fun (_, typ) -> subst_typ subst typ) sd.sfields

let fields_types_fresh sid =
  let lfts = List.map (fun _ -> fresh_lft ()) (get_struct_def sid).slfts in
  subst_fields_types sid lfts, Tstruct (sid, lfts)

let fn_prototype_fresh fid =
  let fd = get_fun_def fid in
  ...
  let rec aux acc = function
    ...
    | Tstruct (id, lfts) ->
        if not (Hashtbl.mem seen (id, lfts)) then (
          ...
          let fields_typs = subst_fields_types id lfts in
