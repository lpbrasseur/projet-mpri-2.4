...
let go lft_sets mir : analysis_results =
  ...
  let module Graph = struct
    ...
    let foreach_successor lbl state go =
      let go next state =
        ...
        go next
          (BSet.filter
             (fun bor -> PpSet.mem (PpLocal next) (lft_sets (get_bor_info mir bor).blft))
             state)
      in

      let assign pl state =
        (* When writing to a place, we de-activate any borrows of any of its sub-place. *)
        BSet.filter (fun bor -> not (is_subplace (get_bor_info mir bor).bplace pl)) state
      in
