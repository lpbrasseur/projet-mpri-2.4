...
val fields_types_fresh : string -> full_typ list * full_typ
...
val fn_prototype_fresh : string -> full_typ list * full_typ * (lifetime * lifetime) list
(** Given a function identifier, instantiate its prototype with fresh lifetime variables,
    then returns the (substituted) types or its parameters, its (substituted) return type,
...
val implied_outlives : full_typ -> outlives_graph
