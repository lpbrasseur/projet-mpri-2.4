...
let check_typ loc check_lvar_num visit_lvar typ =
...
let check_typ_in_decl loc used_lfts typ =
  check_typ loc true
    (fun lft ->
...
let check_struct_decl used_lfts struc =
  ...
  List.iter
    (fun (id, typ) ->
      ...
      check_typ_in_decl struc.sloc used_lfts typ;
      if struc.scopy && not (typ_is_copy typ) then
...
let check_fun_decl used_lfts fd =
  ...
  List.iter
    (fun (param, _, typ) ->
      ...
      check_typ_in_decl fd.floc used_lfts typ)
    fd.fformals;
  check_typ_in_decl fd.floc used_lfts fd.freturn;
...
let () =
  List.iter
    (fun decl ->
      ...
      match decl with
      | Dstruct s -> check_struct_decl used_lfts s
      | Dfundef f -> check_fun_decl used_lfts f)
    ast
...
let check_fun_body fd =
  let not_an_lval exp_lval expr =
    if Option.is_some exp_lval then
      Error.error expr.eloc "This expression should be an l-value."
  in
  let not_mut_lval exp_lval expr =
    if exp_lval = Some Mut then
      Error.error expr.eloc "This expression should be a mutable l-value."
  in
  ...
  and check_infer env ?(exp_lval = None) exp_typ expr =
    let res =
      ...
      | Evar id -> (
          ...
          | mut, ty ->
              if mut = NotMut then not_mut_lval exp_lval expr;
              Some ty
      ...
      | Ederef e -> (
          ...
          match typ with
          | Tborrow (_, Mut, typ) -> Some typ
          | Tborrow (_, NotMut, typ) ->
              not_mut_lval exp_lval expr;
              Some typ
          | _ ->
      ...
      | Econstr_stru (stru, fields) -> (
          not_an_lval exp_lval expr;
          match Hashtbl.find_opt global_env stru.id with
          | Some (Dstruct { sfields; _ }) ->
      ...
      | Ecall (f, params) -> (
          not_an_lval exp_lval expr;
          match Hashtbl.find_opt global_env f.id with
      ...
      | Elet (x, mut, typ, e, b) ->
          not_an_lval exp_lval expr;
          check_typ expr.eloc false (fun () -> ()) typ;
...
let () =
  List.iter (fun decl -> match decl with Dfundef f -> check_fun_body f | _ -> ()) ast
