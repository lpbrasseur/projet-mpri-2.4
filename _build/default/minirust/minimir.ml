...
type rvalue =
  ...
  | RVbinop of binop * local * local
  | RVunop of unop * local
  | RVmake of string * local list
...
let typ_of_place mir pl =
  let rec aux = function
    ...
    match aux pl with
    | Tstruct (s, lfts) ->
        let struc = Ast.get_struct_def s in
        let typ =
          Option.get
          @@ List.find_map
